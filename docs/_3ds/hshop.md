---
author: 3hs
categories:
- utility
color: '#32f661'
color_bg: '#1a8032'
description: The better way to install hShop content on a 3DS.
downloads:
  hShop.3dsx:
    size: 1823872
    size_str: 1 MiB
    url: https://download2.erista.me/3hs/3hs.3dsx
  hShop.cia:
    size: 1737664
    size_str: 1 MiB
    url: https://download2.erista.me/3hs/3hs.cia
icon: https://raw.githubusercontent.com/venkeyz/vendb/master/source/images/hShop.png
image: https://raw.githubusercontent.com/venkeyz/vendb/master/source/images/hShopBanner.png
image_length: 59205
last_updated: 2026-07-02 at 14:03 (UTC)
layout: app
llm_generation: unknown
qr:
  hShop.cia: https://db.universal-team.net/assets/images/qr/hshop-cia.png
stars: 0
systems:
- 3DS
title: hShop
unique_ids:
- '0x1E5E6A2A'
updated: '---'
version: 1.5.11.1
---
